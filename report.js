function generateWorkplaceReport(){
  ...
  const rows=records.map((r,i)=>`
    <tr><td>${i+1}</td><td>${esc(r.inspectionDate)}</td><td>${esc(r.location)}</td>
  ...
  document.getElementById("output").innerHTML=`
    ...
    <table><tr><th>No</th><th>Date</th><th>Location</th><th>Category</th><th>Concern</th>
...
function generateSHOReport(){
  ...
  const rows=selected.map((r,i)=>`
    <tr><td>${i+1}</td><td>${esc(r.location)}</td><td>${esc(r.category)}</td>
  ...
  document.getElementById("output").innerHTML=`
    ...
    <table><tr><th>No</th><th>Location</th><th>Item</th><th>Concern</th>
